...
description: "The Valyent.ts SDK provides a type-safe way to interact with Valyent Cloud, a developers-first cloud platform for deploying and managing containerized applications in microVMs at scale."
---
...
<ResponseField name="MachineConfig" type="object">
  <Expandable title="Properties">
    <ResponseField name="image" type="string" required>
      Docker image to run in the machine
    </ResponseField>
    <ResponseField name="guest" type="object" required>
      <Expandable title="GuestConfig">
        <ResponseField name="cpu_kind" type="string" required>
          Type of CPU allocation
        </ResponseField>
        <ResponseField name="memory_mb" type="number" required>
          Memory allocation in MB (minimum: 1)
        </ResponseField>
    ...
    <ResponseField name="workload" type="object" required>
      <Expandable title="Workload">
        <ResponseField name="env" type="string[]">
          Environment variables
        </ResponseField>
        <ResponseField name="restart" type="object">
          <Expandable title="RestartPolicyConfig">
    ...
    <ResponseField name="stop_config" type="object">
      <Expandable title="StopConfig">
    ...
    <ResponseField name="auto_destroy" type="boolean">
      Whether to automatically destroy the machine when stopped
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="MachineStatus" type="enum">
  - `created`: Initial state after creation - `preparing`: Machine is being
  prepared - `starting`: Machine is starting up - `running`: Machine is running
  - `stopping`: Machine is being stopped - `stopped`: Machine has stopped -
  `destroying`: Machine is being destroyed - `destroyed`: Machine has been
  destroyed
</ResponseField>
